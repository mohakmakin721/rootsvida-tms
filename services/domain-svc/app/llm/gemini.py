...
class GeminiProvider(LLMProvider):
    ...
    def draft(self, brief: DraftBrief) -> ItineraryDraft:
        ...
        prompt = (
            ...
            f"{brief.model_dump_json(indent=2)}\n\n"
            f"{research_block}"
            "Apply the system rules to THIS brief. Checklist: (1) run the 7-category "
            "checklist for EVERY day — no day left with only a narrative; (2) prefer "
            "the highest-scoring candidates; (3) add the origin↔destination main leg "
            "(priced) on day 1 and the last day; (4) add every REQUIRED permit + an "
            "ops_note; (5) add a Visa (misc) if international; (6) obey `notes` "
            "exactly; (7) allocation per the rules (flights/trains + tickets per "
            "person, car/tempo shared); (8) keep the estimated total within "
            "budget_inr, else ops_note the overshoot. Every estimate_amount must be a "
            "realistic current INR price and the day sequence logistically feasible."
        )
